...
class FilesWriterTestCase(unittest.TestCase):
    ...
    def test_write_gzip(self, mock_gzip_open):
        ...
        mock_gzip_open.assert_called_once_with(
            "/test/" + datetime.datetime.now().strftime("%Y%m%d-%H-tweets.txt.gz"),
            "at",
            encoding="utf8",
        )
        ...
        #
        mock_gzip_open.side_effect = OSError(
            errno.EACCES, os.strerror(errno.EACCES), "permission_file"
        )

        with self.assertRaises(
            twitterstreamarchive.exceptions.LocalFileException
        ) as lfe:
            twitterstreamarchive.file_writer.write_gzip(
        ...
        self.assertEqual(
            str(the_exception),
            'Permission error with "permission_file/%s"'
        ...
        self.assertEqual(
            str(the_exception),
            'Unknown error with "io_error/%s": Input/output error'
            % datetime.datetime.now().strftime("%Y%m%d-%H-tweets.txt.gz"),
        )
